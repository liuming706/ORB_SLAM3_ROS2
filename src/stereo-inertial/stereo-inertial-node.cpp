...
void StereoInertialNode::SyncWithImu() {
  ...
  while (1) {
    ...
    if (!imgLeftBuf_.empty() && !imgRightBuf_.empty() && !imuBuf_.empty()) {
      ...
      // 最老的IMU帧时间戳必须老于等于左侧相机帧时间戳
      // 否则，左右两侧相机帧的时间戳都小于最后一帧IMU
      if (tImLeft > Utility::StampToSec(imuBuf_.back()->header.stamp)) continue;
